    ...
    <Container maxWidth="lg" className={classes.container}>   
      <Grid container item direction="row" xs={12} md={12} lg={12}>            
        <Grid item xs={12} md={12} lg={12} justify = "center"> 
            ...
            {success ?<Alert variant="filled" severity="success"> Berhasil menghapus satuan ! </Alert> : ''}
            ...
            <MaterialTable 
                ...
                editable={{                 
                  onRowDelete: async oldData => {            
                    ...
                    const delData = await axios.post(`${host}api/v1/satuan/del/${oldData.id}`)
                      if(delData.status == 200){
                        const dataDelete = [...satuan];
                        const index = oldData.tableData.id;
                        dataDelete.splice(index, 1);
                        setSatuan(dataDelete);                        			
                      }        
                  }
                }}
                actions={[
                  {
                    ...
                    tooltip: 'Tambah',
                    onClick: () => router.push('/manage/satuan/add'),
                    isFreeAction: true
                  },
                  {
                    icon: 'edit',

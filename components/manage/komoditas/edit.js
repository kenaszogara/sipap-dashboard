...
export default function Add({ data }) {
    ...
    const editkomoditas = async (formData) => {    
        setLoading(true)
        ...
        const addData = await axios.post(`${host}api/v1/komoditas/edit/${data.id}`, sendData)
        if(addData.status == 200){
            setLoading(false)                
            setSuccess(true)   
            router.push('/manage/komoditas/')      
        }else{
            setError(true)
            setLoading(false)
    ...
    <Box bgcolor="#fff" boxShadow={3} borderRadius={16} style={{ padding: "3em" }}>
        ...
        <Grid container item direction="row" xs={12} md={12} lg={12}>            
            <Grid item xs={12} md={12} lg={12}>
                <h2>Ubah komoditas</h2>                 
            </Grid>
        </Grid>      
        <form onSubmit={handleSubmit(editkomoditas)}>
        <Grid container item direction="row" xs={12} md={12} lg={12}>
            <Grid item xs={12} md={12} lg={12}>                
